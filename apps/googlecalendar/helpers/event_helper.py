...
def __create_event_dtos(event_lists):
    ...
    for event_list in event_lists:
        for event in event_list["items"]:
            ...
            event_dto = EventDto(etag=event.get("etag"), event_id=event.get("id"), summary=event.get("summary"), location=event.get("location"), display_name=event_display_name, start_time=event_start_time, end_time=event_end_time)
